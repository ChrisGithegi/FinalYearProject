...
mycursor.execute("LOAD DATA LOCAL INFILE '../data/ProcessedData.csv' INTO TABLE traffic FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 ROWS (id, UniqueID, @Date, Time, Site, Type, Make, Generic_Model, Fuel_Type, Body_Type, Body_Type_Detail, @Date_Reg, co2, Mass) SET Date = STR_TO_DATE(@Date, '%d/%m/%Y'), Date_Reg = STR_TO_DATE(@Date_Reg, '%Y-%m-%d')")
